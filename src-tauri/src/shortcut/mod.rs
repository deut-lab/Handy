...
#[tauri::command]
#[specta::specta]
pub fn change_overlay_button_style_setting(
    app: AppHandle,
    button_style: String,
) -> Result<(), String> {
    let mut settings = settings::get_settings(&app);
    let parsed = match button_style.as_str() {
        "simple" => OverlayButtonStyle::Simple,
        "circle" => OverlayButtonStyle::Circle,
        other => {
            warn!(
                "Invalid overlay button style '{}', defaulting to circle",
                other
            );
            OverlayButtonStyle::Circle
        }
    };
    settings.overlay_button_style = parsed;
    settings::write_settings(&app, settings);
    Ok(())
}
